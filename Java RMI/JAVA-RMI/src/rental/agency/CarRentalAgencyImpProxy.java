...
public class CarRentalAgencyImpProxy implements CarRentalAgency, CompanyProvider {

    public CarRentalAgencyImpProxy(Registry registry) {
    ...
    public ReservationSession createReservationSession(String clientName) throws RemoteException {
        if  (!renterSessions.containsKey(clientName)) {
            ...
            ReservationSession sessionProxy = (ReservationSession)Proxy.newProxyInstance(
                    ReservationSession.class.getClassLoader(),
                    new Class[] {ReservationSession.class},
                    new SessionInvocationHandler(session));
            ReservationSession stub = (ReservationSession) UnicastRemoteObject.exportObject(sessionProxy, 10124);
    ...
    public ManagerSession createManagerSession(String managerName, String rentalName) throws RemoteException {
        if  (!managerSessions.containsKey(managerName)) {
            ...
            ManagerSession sessionProxy = (ManagerSession)Proxy.newProxyInstance(
                    ManagerSession.class.getClassLoader(),
                    new Class[] {ManagerSession.class},
                    new SessionInvocationHandler(session));
            ManagerSession stub = (ManagerSession)UnicastRemoteObject.exportObject(sessionProxy, 10124);
